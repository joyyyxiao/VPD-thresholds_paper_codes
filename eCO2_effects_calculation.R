...
# ========= data input ===========
# ls <- paste0("./NIRv_monthly/NIRvMean",rep(2000:2022,each=12),"_",rep(1:12,23),".tif")[-1]
# r_nirv <- stack(ls)
# line <- read.csv("line.csv")[,-1]
# dfnirv <- matrix(NA,nrow = 259200,ncol=275)
# for (i in 1:275){
# dfnirv[,i] <- as.numeric(as.matrix(subset(r_nirv,i)))
# }
# rownames(dfnirv) <- line
# write.csv(dfnirv,"./data/NIRv/dataframe_NIRv.csv")

# line <- read.csv("line.csv")[,-1]
# time <- ts(data=NA,start(2000,2),freq=12)
VIdf <- read.csv("./data/NIRv/dataframe_NIRv.csv",row.names=1, check.names=FALSE)
...
CO2ts <- read.csv("./data/co2_annmean_mlo.csv")# CO2 time series
CO2ts <-  ts(CO2ts[,2], start=c(1959, 1), end=c(2022,1), frequency = 1)#annual
...
franksCO2a <- function (CTSR.VI, C4frac, CO2 = FALSE, refyear = 1980) {
  ...
  franks_FvC <- function(Ca) {
    theta = 0.7
    gamma_star = 40
    return((theta * Ca - gamma_star)/(theta * Ca + 2 * gamma_star))
  }
  baseline_Ca = window(CO2, refyear, refyear + 1)[[1]]
  baseAnet = franks_FvC(baseline_Ca)
  yst <- start(CTSR.VI)[1]
...
tssr.attr.annual.ppm <- function(line, VI, C4frac,par){
  # =========== Function is applied to one pixel at a time ===========
  # ========== Perfrom the data checks and if anything fails skipp processing ==========
  # There is a data check for NANs in the TSSRattribution function, If SkipError is True
  # It then returns an opject of the same structure as actual results but filled with NaN
  ...
  if (any(is.na(VI)) | (sd(VI)==0)){
    ...
    CO2_dif <- function(CO2,start,end,refyear=1980){
      ...
      # # ========== get the CO2 concentration of the dataset years ==========
      # +++++ build a matching CO2 series +++++
      Ca <- as.numeric(window(CO2, start, end))
      dif <- Ca-baseline_Ca
      return(dif)
    }
    ...
    # ========== Calculate CO2 Change ==========
    co2dif <- CO2_dif(CO2=CO2ts,start=2000,end=2022,refyear=1980)
    CO2change <- (rawmax.df$Max - adjmax.df$Max)/co2dif
    results <-  CO2change
  }
  # ========== return the results ==========
  ret <- t(results)
  rownames(ret) <- line         # add the row name back
  colnames(ret) <- c(2000:2022)
  return(ret)
}
...
ptime  <- system.time(
  tss.atdf <- foreach(
    ...
    .packages = c('TSS.RESTREND', "xts", "lubridate","mblm")) %dopar% {
      tssr.attr.annual.ppm(line, VIdf[line, ],C4df[line, ], par=TRUE)
    })
...
###save data###
write.csv(tss.atdf,"./results/NIRv_annual_ppm_CO2_effect.csv")
